/**
 * LifeXP push server — the piece that makes reminders arrive even when
...
function requireAuth(req, res, next) {
  ...
  if (req.headers['x-lifexp-secret'] !== SHARED_SECRET) {
    return res.status(401).json({ error: 'unauthorized' });
  }
  next();
}

app.get('/', (req, res) => res.json({ ok: true, service: 'lifexp-push-server' }));
...
app.listen(PORT, () => console.log('LifeXP push server listening on :' + PORT));
