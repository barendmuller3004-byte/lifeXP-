...
// Tapping a reminder focuses an existing LifeXP tab if one's open,
...
// no LifeXP tab is open at all — that's the entire point of it existing).
self.addEventListener('push', (event) => {
  let data = { title: 'LifeXP', body: '' };
  try {
  ...
  event.waitUntil(
    self.registration.showNotification(data.title || 'LifeXP', { body: data.body || '' })
  );
});
