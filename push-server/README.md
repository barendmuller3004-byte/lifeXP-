...
The one piece that makes reminders arrive even when LifeXP (and the
browser) are completely closed. Everything else in LifeXP runs with no
...
(e.g. `https://your-app.onrender.com`) — that's what goes into LifeXP.
...
## 4. Connect LifeXP to it

In LifeXP: **Settings → Developer → Push Notifications**
...
LifeXP will fetch the VAPID public key from your server automatically,
subscribe this browser to push, and send it. From then on, whenever a
reminder's schedule changes locally, LifeXP re-syncs the list to your
...
LifeXP data goes anywhere. If that trade-off isn't worth it for you,
don't set this up — everything else in LifeXP still works exactly the
